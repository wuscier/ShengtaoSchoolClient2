...
namespace St.Host
{
    ...
    public partial class App
    {
        ...
        private void KeepLatestTwoVersions()
        {
            try
            {
                DirectoryInfo curDi = new DirectoryInfo(Environment.CurrentDirectory);

                var dis = curDi.Parent.GetDirectories("app-*");

                Log.Logger.Debug($"count of apps：{dis}");

                if (dis != null && dis.Count() > 2)
                {
                    int oldCount = dis.Count() - 2;
                    Log.Logger.Debug($"count of old apps：{oldCount}");
                    var oldDis = dis.OrderBy(di => di.CreationTime).Take(oldCount);
                    foreach (var oldDi in oldDis)
                    {
                        Log.Logger.Debug($"deleting {oldDi.FullName}");
                        oldDi.Delete(true);
                    }
                }
            }
                ...
                using (var mgr = new UpdateManager(updateUrl))
                {
                    SquirrelAwareApp.HandleEvents(
                        ...
                        onAppUpdate:  v =>
                        {
                            ...
                            CopyConfigFile(v.ToString(3));
                            KeepLatestTwoVersions();
                            CreateShrotcut();
                            Current.Shutdown();
                        },
